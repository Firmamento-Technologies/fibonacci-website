 ...
 *
 * ── PERCHÉ `manuale-lingua` E NON `manuale-privato/[lingua]` ────────────────
 * Perché `app/manuale-privato/route.ts` scrive un **file** in
 * `out/manuale-privato`, e una sotto-rotta lì dentro pretenderebbe che lo
 * stesso nome fosse una **cartella**. Provato: la build si ferma con
 * «EISDIR: illegal operation on a directory, copyfile … -> out/manuale-privato».
 * ⇒ un percorso proprio, e i due non si pestano i piedi.
 *
...
export async function GET(_req: Request, ctx: { params: Promise<{ lingua: string }> }) {
  ...
  return NextResponse.json({
    generato_da: 'website/src/app/manuale-lingua/[lingua]/route.ts',
    pubblico: false,
    lingua: l,
