...
- **Categoria di servizio**: DNS autoritativo per il dominio `il sito Fibonacci` ed i relativi sottodomini, proxy HTTPS, rete di distribuzione di contenuti (CDN) per gli asset statici dell'applicazione, protezione contro attacchi distribuiti (DDoS), Web Application Firewall (WAF)
