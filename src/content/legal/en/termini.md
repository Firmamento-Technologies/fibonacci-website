...
| REA Number | {REA} |
| Certified Email | {PEC} |
| Commercial Contact | {EMAIL_COMMERCIALE} |
| Support Contact | {EMAIL_SUPPORTO} |
...
1.1 **Provider**: {DENOMINAZIONE}, with registered office in {SEDE_LEGALE}, VAT number {PARTITA_IVA}.
...
1.4 **Service**: the "Fibonacci" SaaS software, delivered in cloud mode and accessible via web browser at `{URL_APP}`, including the modules, functionalities, documentation, and ancillary services specified in the subscribed Plan.
...
1.11 **DPA**: the Data Processing Agreement published at `{URL_SITO}/dpa`, governing the roles and obligations of the parties regarding the processing of personal data under Regulation (EU) 2016/679.
...
2.2 The Service is accessible via web browser at `{URL_APP}` and is hosted exclusively on infrastructure under the Provider’s responsibility. This contract does not include an on-premise installation license or delivery of the source code.
...
3.1 The contract is concluded upon the Client’s electronic acceptance of these General Conditions by ticking the appropriate box during registration at `{URL_APP}`, simultaneously entering identification and payment details, or, in the case of Trial Period activation, at its commencement.
...
| Plan | Monthly fee (VAT excluded) | Operators | Main features |
|---|---|---|---|
| Solo | 99.00 {{euro}} | 1 doctor | Unlimited patients, medical records, body-map, consents, electronic signature, preservation |
| Studio | 189.00 {{euro}} | up to 5 operators | Dictation during visits, shared calendar, appointment reminders |
...
6.3 Within 30 (thirty) days of receiving the modification notice, the Client may terminate the contract without costs or penalties by notifying {EMAIL_SUPPORTO} via email or through the "Subscription" panel of the application. Failure to notify termination within this period constitutes tacit acceptance of the modification.
...
8.4 The service credit is applied to the next available invoice upon written request from the Client to be sent to {EMAIL_SUPPORTO} within 30 (thirty) days from the end of the reference month, under penalty of forfeiture. The credit is not cumulative with other remedies and is the Client’s sole compensation for SLA non-compliance.
...
10.1 The processing of personal data, including Patient Data, stored, processed, or transmitted through the Service is governed by the Data Processing Agreement (DPA) published at `{URL_SITO}/dpa`, an integral part of this contract, which the Client declares to know and accept upon subscription.
...
10.4 The Provider’s Privacy Policy regarding processing as Data Controller (e.g., for Client Data, cookies, commercial communications) is published at `{URL_SITO}/privacy`.
...
14.2 The export request must be sent to the Provider via email at {EMAIL_SUPPORTO} within 30 (thirty) days from the contract termination date.
...
15.4 If a modification significantly reduces the essential functionalities of the Client’s subscribed Plan, the Client may terminate the contract without costs or penalties within 30 (thirty) days from the communication, by notifying {EMAIL_SUPPORTO} via email.
...
18.3 The Provider’s email addresses are: {EMAIL_COMMERCIALE} for commercial and contractual communications, {EMAIL_SUPPORTO} for technical support requests.
...
21.3 This document, together with the Data Processing Agreement published at `{URL_SITO}/dpa` and the Privacy Policy published at `{URL_SITO}/privacy`, constitutes the entire agreement between the parties regarding the contract’s subject matter and supersedes any prior written or oral understanding on the same subject.
