...
- a) **Aruba S.p.A.**, con sede in Via San Clemente 53, 24036 Ponte San Pietro (BG), Italia. Attività: hosting dell'infrastruttura applicativa e dei database del Software. Infrastruttura su rete italiana, sotto giurisdizione dell'Unione Europea. Trasferimenti extra-UE: nessuno.
- b) **Hostinger International Ltd**, con sede in 61 Lordou Vironos str., 6023 Larnaca, Cipro. Attività: recapito di email transazionali tramite server SMTP autenticato (conferme di appuntamento, notifiche di sistema, primo accesso) e DNS autoritativo per il dominio `{URL_SITO}` e i relativi sottodomini. Dati trattati: indirizzo email dei destinatari e contenuto del messaggio. Non riceve dati clinici di dettaglio nel corpo del messaggio.
...
⚠️ **Correzione rispetto alle versioni precedenti del presente DPA.** L'elenco indicava alla lettera a) un fornitore di hosting tedesco e alla lettera e) un fornitore statunitense di DNS, proxy e rete di distribuzione di contenuti. Il primo è stato sostituito dal soggetto effettivo, il secondo è stato eliminato perché il Servizio **non si avvale di alcun intermediario di rete**: il dominio risolve direttamente sull'infrastruttura di cui alla lettera a). Entrambe le circostanze sono verificabili dall'esterno con una interrogazione DNS sul dominio del Servizio e una interrogazione `whois` sull'indirizzo risultante. Il dettaglio della correzione è nel registro versioni dell'Allegato B.
...
8.1. I Dati personali e, in particolare, i dati sanitari di cui all'art. 9 GDPR non sono trasferiti al di fuori dell'Unione Europea. Il database produttivo, i backup e le foto cliniche risiedono fisicamente in Italia.

8.2. **Nessuna entità con sede negli Stati Uniti d'America è inclusa nella filiera che tratta i dati dei pazienti.** Fra il browser del Titolare e il backend non è interposto alcun intermediario: non sono impiegate reti di distribuzione di contenuti, proxy inversi di terze parti o Web Application Firewall gestiti da terzi, e la connessione cifrata è terminata unicamente dal reverse proxy del Responsabile.

8.3. L'unico Sub-responsabile con repliche di resilienza al di fuori dell'Unione Europea è quello indicato alla lettera d) dell'art. 7.2, il quale **non tratta dati dei pazienti** e opera su una filiera segregata da quella clinica. Per tale fornitore le Parti danno atto che il trasferimento avviene sulla base delle clausole contrattuali tipo di cui alla Decisione UE 2021/914, integrate dalle misure supplementari conformi alle Raccomandazioni 01/2020 dell'EDPB.
